...
class PurchaseListener : EmptyRequestListener<Purchase>() {
    override fun onSuccess(purchase: Purchase) {
        // here you can process the loaded purchase
        Timber.tag("pay").d(purchase.toString())
        removeAds()
    }

    override fun onError(response: Int, e: Exception) {
        // handle errors here
        Timber.tag("pay").d(response.toString())
        Timber.e(e)
    }
}
...
class InventoryCallback : Inventory.Callback {
    override fun onLoaded(products: Inventory.Products) {
        // your code here
        Timber.tag("pay").d(products.toString())
    }
}
